...
// Implementierung der Info-Anzeige.
...
// Implementierung der Angriffsoptions-Anzeige.
...
// Implementierung der Schadensverarbeitung.
...
bool Character::useOwp(int cost) {
    if (canAfford(cost)) {
        current_owp -= cost;
        current_owp = std::max(0, current_owp);
        return true;
    }
    return false;
}
